...
if (isset($_SESSION['login'])) {
	...
	$tanggla_input = date ('Y-m-d H:i:s');
	$user_id     = ($_SESSION['user_id']); 
	if (!empty($nama) && !empty($alamat) && !empty($tempat) && !empty($tanggal) && !empty($jenis)){
		mysqli_query($connect, "
		UPDATE santri SET nama = '$nama', tempat_lahir = '$tempat', tanggal_lahir = '$tanggal', alamat = '$almat', jenis_kelamin = '$jenis' WHERE id = '$id'");
		header("location:../santri.php");
	}else {
		echo "Selahkan lengkapi data <a href='../santri_tambah.php'>santri</a>";
	}
}else { 
